...
# **1行目が数字であることを確かめてから使う。**shell と python の間の
# 契約は「1行目が件数」という位置だけの約束で、型は保証されない。
# python3 が PATH にあっても空文字を吐いて exit 0 するだけの壊れ方
# （traceback も出ない）だと `result` の1行目も空になり、確かめずに
# `[ "$pp3_count" -eq 0 ]` へ渡すと bash が「整数の式が予期されます」と
# エラーを出して**その比較自体が偽**になる。`set -e` が無いので分岐は
# 抜けず、$failed も空のまま「PP3xx: 指摘なし」で exit 0 に落ちる
# （実測: PP302 が実際に fail している状態でも通ってしまう）。
# これは「解析できなかった」（上の分岐）とも「0件評価」とも別の故障
# なので、専用のメッセージで区別する。
case "$pp3_count" in
    ''|*[!0-9]*)
        echo "PP3xx の件数行を読めませんでした（解析器の出力が壊れています）: $repo" >&2
        exit 1
        ;;
esac
