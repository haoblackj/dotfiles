...
out="$(printf '%s' "$INPUT1" | bash "$SCRIPT")"; rc=$?
...
out="$(printf '%s' "$INPUT2" | bash "$SCRIPT")"; rc=$?
...
out="$(printf '%s' "$INPUT3" | bash "$SCRIPT")"; rc=$?
...
out="$(printf '%s' "$INPUT4" | bash "$SCRIPT")"; rc=$?
...
for bad in "0" '"notanumber"' "-5"; do
  ...
  printf '%s' "$input" | bash "$SCRIPT" >/dev/null
